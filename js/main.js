...
// Isso garante que todos os elementos do formulário já existem na página.
document.addEventListener('DOMContentLoaded', function() {

  // Inicializa o EmailJS com a sua Public Key.
  // É importante que isso seja feito antes de qualquer tentativa de envio.
  emailjs.init({
    publicKey: "SLS8SEfkneyGnCM83", // Substitua pela sua Public Key do EmailJS
  });

  // Pega as referências dos elementos do formulário no HTML.
  ...
  if (!contactForm || !submitButton || !formStatus) {
    console.error("Não foi possível encontrar os elementos do formulário. Verifique os IDs no HTML.");
  ...
  contactForm.addEventListener('submit', function(event) {
    ...
    // ---- 1. Mudar para o estado de "Enviando" ----
    const originalButtonText = submitButton.textContent;
    submitButton.disabled = true;
    submitButton.textContent = 'Enviando...';
    formStatus.textContent = ''; // Limpa a mensagem de status anterior.
    formStatus.className = '';   // Remove as classes de cor (verde/vermelho).

    // IDs do seu serviço e template no EmailJS.
    const serviceID = 'service_c9pnfon';
    const templateID = 'template_h0zi9nq';

    // ---- 2. Enviar o formulário usando emailjs.sendForm ----
    // A estrutura .then/.catch/.finally é ótima para lidar com promessas.
    emailjs.sendForm(serviceID, templateID, this)
      .then(() => {
        // ---- 3. Lidar com o sucesso ----
        formStatus.textContent = 'Mensagem enviada com sucesso! Entraremos em contato em breve.';
        formStatus.className = 'status-success'; // Adiciona a classe para a cor verde.
      ...
      .catch((err) => {
        ...
        formStatus.className = 'status-error'; // Adiciona a classe para a cor vermelha.
        console.error('Erro ao enviar email:', JSON.stringify(err));
      })
      .finally(() => {
        // ---- 5. Restaurar o estado original do botão ----
